...
import { Shield, Mail, Lock, CheckCircle, ArrowLeft, Key, Copy } from 'lucide-react';
...
export default function ForgotPassword() {
  ...
  // Auto-load token from sessionStorage if navigating from the "Use this token" link
  const storedToken = sessionStorage.getItem('sq_reset_token');
  const [showResetForm, setShowResetForm] = useState(!!storedToken);
  ...
  // If we have a token or user clicked to show reset form, show reset password form
  if (resetToken || resetSuccess || showResetForm) {
    return (
      <>
        <AuthHeader />
                      ...
                      <div className="flex-1">
                        ...
                        {resetToken && (
                          <div className="mt-3 p-4 bg-white rounded-xl border border-green-200 shadow-sm">
                            ...
                            <code className="text-sm font-mono text-slate-800 bg-slate-100 px-3 py-2 rounded-lg block break-all border border-slate-200">
                              {resetToken}
                            </code>
                            ...
                            <p className="mt-2 text-xs text-slate-500">
                              Note: In production, this token would be sent via email. For now, copy it and use it above.
                            </p>
                          </div>
                        )}
                      </div>
